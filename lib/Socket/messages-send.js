    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (content && typeof content === 'object') {
                const hasPin = 'pin' in content;
                const hasUnpin = 'unpin' in content;

                if (hasPin || hasUnpin) {
                    const isUnpinAction = hasUnpin;

                    // Support 2 format:
                    // 1. { pin: { key, time } } / { unpin: { key } }
                    // 2. { pin: key, time } / { unpin: key }
                    const rawPinData = isUnpinAction ? content.unpin : content.pin;

                    let key;
                    let duration;

                    if (rawPinData && typeof rawPinData === 'object' && 'key' in rawPinData) {
                        key = rawPinData.key;
                        duration = rawPinData.time;
                    } else {
                        key = rawPinData;
                    ...
                    const pinMessage = {
                        ...
                        messageContextInfo: {
                            messageAddOnDurationInSecs: isUnpinAction ? 0 : (duration || 86400)
                        }
                    };
