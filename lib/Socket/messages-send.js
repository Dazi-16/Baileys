...
const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // ===== PATCH: HANDLE PIN / UNPIN MESSAGE =====
            if (content && typeof content === 'object') {
                const isPinObject =
                    content.pin &&
                    typeof content.pin === 'object' &&
                    content.pin.key;

                const isPinKeyStyle =
                    content.pin &&
                    typeof content.pin === 'object' &&
                    content.pin.remoteJid &&
                    content.pin.id;

                const isUnpinObject =
                    content.unpin &&
                    typeof content.unpin === 'object' &&
                    content.unpin.key;

                const isUnpinKeyStyle =
                    content.unpin &&
                    typeof content.unpin === 'object' &&
                    content.unpin.remoteJid &&
                    content.unpin.id;

                if (isPinObject || isPinKeyStyle || isUnpinObject || isUnpinKeyStyle) {
                    const isUnpin = !!content.unpin;

                    const pinPayload = isUnpin
                        ? (isUnpinObject ? content.unpin : { key: content.unpin })
                        : (isPinObject
                            ? content.pin
                            : {
                                key: content.pin,
                                time: content.time || 86400,
                                type: 1
                            });

                    const pinType = isUnpin
                        ? (pinPayload.type || 2)
                        : (pinPayload.type || 1);
                    ...
                    if (!isUnpin && pinPayload.time) {
                        pinMessage.messageContextInfo.messageAddOnDurationInSecs = pinPayload.time;
                    }

                    const msgId = options?.messageId || Utils_1.generateMessageID();

                    console.log('===== PIN PATCH DEBUG =====');
                    console.log(JSON.stringify({
                        jid,
                        isUnpin,
                        pinPayload,
