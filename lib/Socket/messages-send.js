...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList, AI = true }) => {
        // ===== HANDLE PIN MESSAGE DIRECTLY =====
        // CEK APAKAH INI PESAN PIN
        if (message && message.pinInChatMessage) {
            const pinData = message.pinInChatMessage;
        
            console.log('📌 PROCESSING PIN MESSAGE:', {
                jid,
                key: pinData.key,
                type: pinData.type
            });
            ...
            // Validasi key
            if (!pinData.key || !pinData.key.id) {
                throw new Error('Invalid pin key: missing id');
            }
            // Buat stanza pin
            const pinStanza = {
                tag: 'iq',
                ...
                content: [{
                    ...
                    attrs: {
                        chat: jid,
                        id: pinData.key.id,
                        action: pinData.type=== 2 ? 'unpin' : 'pin'
                    }
                }]
            };

            console.log('📌 SENDING PIN STANZA:', JSON.stringify(pinStanza, null, 2));

            try {
                // Kirim stanza
                await sendNode(pinStanza);
                // Generate message ID kalo belum ada
                const finalMsgId = msgId || Utils_1.generateMessageID();
                ...
                console.error('📌 PIN SEND ERROR:', error);
                throw error;
            }
        }
        // Tunggu response
        try {
            const response = await Promise.race([
                ...
                (async () => {
                    // Listen for iq response
                    return new Promise(resolve => {
                        const listener = (node) => {
                            if (node.tag === 'iq' && node.attrs.from === jid) {
                                ev.off('node', listener);
                                resolve(node);
                            }
                        };
                        ev.on('node', listener);
        ...
        const devices = [];
        const isPinMessage = !!message?.pinInChatMessage;
        // ===== PATCH PIN SUPPORT (TAMBAHKAN INI) =====
        if (isPinMessage) {
            const pinData = message.pinInChatMessage;
    
            // Buat stanza khusus untuk pin
            const pinStanza = {
                tag: 'iq',
                attrs: {
                    to: jid,
            ...
            // Kembalikan msgId, jangan lanjut ke proses biasa
            return msgId;
        }
        // ===== END PATCH =====
        const meMsg = isPinMessage
            ? message
            : {
                deviceSentMessage: {
                    destinationJid,
        ...
        await authState.keys.transaction(async () => {
            ...
            // KALAU INI PIN MESSAGE, KIRIM JUGA STANZA PIN
            if (message.pinInChatMessage) {
                ...
                console.log('📌 SENDING ADDITIONAL PIN STANZA:', JSON.stringify(pinStanza, null, 2));
                await sendNode(pinStanza);
            }
            ...
            if (message.pinInChatMessage) {
                console.log('==== PIN BINARY CONTENT ====');
                console.log(JSON.stringify(binaryNodeContent, null, 2));
                console.log('==============================');

                console.log('==== FINAL PIN STANZA ====');
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // ===== PATCH: HANDLE PIN / UNPIN MESSAGE (FINAL VERSION) =====
            if (content && typeof content === 'object') {
                // CEK APAKAH INI PIN ATAU UNPIN
                const isPin = content.pin && content.pin.key;
                const isUnpin = content.unpin && content.unpin.key;

                if (isPin || isUnpin) {
                    const pinData = isPin ? content.pin : content.unpin;
                    const isUnpinAction = !!isUnpin;

                    // Validasi key
                      if (!pinData.key || !pinData.key.id || !pinData.key.remoteJid) {
                        throw new Error('Invalid pin key: missing required fields');
                      }
                      ...
                      const pinMessage = {
                        pinInChatMessage: {
                            key: pinData.key,
                            type: isUnpinAction ? 2 : 1,
                            senderTimestampMs: Date.now()
                        }
                    };

                    // Tambah durasi kalo pin
                    if (!isUnpinAction && pinData.time) {
                        pinMessage.messageContextInfo = {
                              messageAddOnDurationInSecs: pinData.time
                        };
                    }

                    const msgId = options?.messageId || Utils_1.generateMessageID();

                    console.log('===== PIN SEND =====');
                    console.log('Action:', isUnpinAction ? 'UNPIN' : 'PIN');
                    console.log('Target JID:', jid);
                    console.log('Key:', JSON.stringify(pinData.key, null, 2));
                    console.log('====================');

                    // PAKAI RELAY MESSAGE LANGSUNG
                    // PIN SUDAH DI-HANDLE DI RELAYMESSAGE
                    await relayMessage(jid, pinMessage, {
                        messageId: msgId,
