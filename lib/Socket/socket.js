...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairKey) => {
        ...
        if (!cleanedNumber) {
            throw new boom_1.Boom('Phone number is required for pairing code', {
                statusCode: 400
            });
        }
        ...
        // PRIORITAS:
        // 1. pairKey dari user
        // 2. default pairing code dari fork
        const finalPairKey = String(pairKey || defaultPairingCode).trim().toUpperCase();

        authState.creds.pairingCode = finalPairKey;

        authState.creds.me = {
            id: (0, WABinary_1.jidEncode)(cleanedNumber, 's.whatsapp.net'),
            name: '~'
        };
